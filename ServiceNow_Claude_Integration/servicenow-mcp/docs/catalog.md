...
This document provides information about the ServiceNow Service Catalog integration in the ServiceNow REST API.
...
The following operations are available for interacting with the ServiceNow Service Catalog:
...
With the backend running, the same catalog operations are reachable directly:

```bash
curl "http://127.0.0.1:8095/catalog/items?limit=5"
curl "http://127.0.0.1:8095/catalog/categories?limit=5"
curl "http://127.0.0.1:8095/catalog/recommendations"
```
